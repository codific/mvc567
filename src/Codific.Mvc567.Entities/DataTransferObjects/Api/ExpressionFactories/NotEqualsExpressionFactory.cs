...
{
    public class NotEqualsExpressionFactory : AbstractExpressionFactory
    {
        public override Expression<Func<TEntity, bool>> BuildExpressionByQueryStringItem<TEntity>(FilterSearchQueryItem queryStringItem)
        {
            ...
            {
                expression = x => propertyInfo.GetValue(x) != null && propertyInfo.GetValue(x).ToString().ToLower() != queryStringItem.Value.ToLower();
            }
