...
namespace Codific.Mvc567.Services.Abstractions
{
    ...
    {
        Task<IEnumerable<TEntityDto>> GetAllEntitiesAsync<TEntity, TEntityDto>()
            where TEntity : class, IEntityBase, new();

        Task<TEntityDto> GetEntityAsync<TEntity, TEntityDto>(Guid id)
            where TEntity : class, IEntityBase, new();

        Task<PaginatedEntitiesResult<TEntityDto>> GetAllEntitiesPaginatedAsync<TEntity, TEntityDto>(int page, string searchQuery = null, string sortProperty = null)
            where TEntity : class, IEntityBase, new();

        Task<Guid?> CreateEntityAsync<TEntity, TEntityDto>(TEntityDto entity)
            where TEntity : class, IEntityBase, new();

        Task<Guid?> ModifyEntityAsync<TEntity, TEntityDto>(Guid id, TEntityDto modifiedEntity)
            where TEntity : class, IEntityBase, new();

        Task<bool> DeleteEntityAsync<TEntity>(Guid id, bool softDelete = true)
            where TEntity : class, IEntityBase, new();

        Task<bool> RestoreEntityAsync<TEntity>(Guid id)
            where TEntity : class, IEntityBase, new();
